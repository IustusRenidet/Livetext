...
const badWords = ['puta', 'puto', 'pendejo', 'mierda', 'cabron', 'idiota', 'fuck', 'shit', 'bitch', 'asshole'];
...
async function createComment(db, postId, commentData, visitorId) {
  ...
  if (containsBadWords(sanitizedContent)) throw new Error('Lenguaje ofensivo no permitido.');
  const comment = {
...
app.post('/api/chat', async (req, res) => {
  ...
  try {
    ...
    if (containsBadWords(lastUserMsg)) {
      return res.status(400).json({ error: 'Lenguaje ofensivo no permitido' });
    }
